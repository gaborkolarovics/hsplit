...
// source inspiration: https://github.com/bobg/hashsplit
...
const (
	...
	defaultMinBytesOfPart = windowSize
)
...
func (s *Splitter) checkSplit() (uint, bool) {
	...
	minBytesOfPart := s.MinBytesOfPart
	if minBytesOfPart == 0 {
		minBytesOfPart = defaultMinBytesOfPart
	}
	h := s.rs.Sum32()
	tz := uint(bits.TrailingZeros32(h))
	if tz >= splitBits && len(s.chunk) > minBytesOfPart {
